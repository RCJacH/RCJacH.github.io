...
没办法，大编制听起来就是爽，但随之而来的就是更多的工作量。

除非你是John Williams或者Howard Shore级别，
大编制同期实录后还能赚一笔，
还有愿意为修改重录买单的甲方，
否则你还是得窝电脑面前老老实实地给虚拟乐器（别名音源）画包络去。

但实际上音源用起来很困难。

首先它们很贵－呃，我们都是良民，肯定是要买正版的啊，eventually。
其次每款音源的麦克风配置、音量校准及部分MIDI控制器的设置可能都不一样。
再次，为了模拟实录听感或者追求的大气厚重的声音，
我们经常需要结合不同的音源来取长补短。
综合下来，统一设置很费时费力。

当然，你可以反驳说音源不需要再次设置，安装完直接用即可。
我之前也是这么想的，在浪费掉几百个小时之后发现好像有点不对。
主要原因是，虽然直接使用可以马上输出音乐内容，
...
其实搭建为虚拟交响乐团搭建一个大型模板已经成为业内的常规操作。
这个模板会包含所有所用音源的所有技法，
通常用到系统的命名方案，
用不同颜色区分乐器组，
预配置的效果器，
...
[storyteller的REAPER交响超级大模板](http://otr.storyteller.im/)
...
这种操作我称之为**单一模板法**。
它几十年前就成为业界标准操作了，至今仍被许多活跃的作曲家（例如Blakus和Mike Verta）所使用。
...
如果你是个REAPER用户，
你应该知道REAPER可以设置成每n分钟自动备份你的工程文件。
鉴于丢失进度是准时交工最大的阻碍，
我把备份间隔设置为每5分钟保存一次。
...
你可以把所选的轨道，包含的MIDI块儿，以及轨道上的插件保存成一个**轨道模板**。
这个*轨道模板*可以随时导入至之后的工程，
就省略掉打开整个大模板的时间。
...
1. 分组容器。
   ...
   带着像Airwindows的Console系列或者总线压缩等相关的效果器。
   然后仅保存这些母轨道为一个模板。
2. 音源。
   第二组模板仅保存加载着MIDI效果器和音源插件（例如Kontakt）的轨道，
   并且最好是以制造商、技法或任何你觉得能区分的元素来单独保存。
   最佳状态是加载模板后不需要再进行任何删除处理，
   以节省加载时间。
3. 音频效果器。
...
所以每次开机仅需要加载一次音源。
同时由于你的主要工作都回归到MIDI上－所有配置都靠MIDI完成，
而插件崩溃最大帮凶反而是GUI，
使用这个模板反而降低了崩溃重启的机率。

它本质上是单一模板法去掉了上述缺点。
同时也避免了模块模板的缺点－对模板进行的任何改动立即反馈至所有相关工程中。
